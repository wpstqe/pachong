...
def postNovelMethod(value):
    result = requests.post(url='http://kefu.baidu.com/novel/default/index', data=value)
    print(result.text)
    try:
...
def getNovelMethod(value):
    result = requests.post(url='http://kefu.baidu.com/novel/default/search', data=value)
    print(result.text)
    try:
        data = json.loads(result.text)
        print(data['data'])
...
def postChapterMethod(value):
    result = requests.post(url='http://kefu.baidu.com/novel/chapter/index', data=value)
    print(result.text)
    try:
        data = json.loads(result.text)
        print(data)
...
baseUrl = 'https://m.guwenxue.org'
url = 'https://m.guwenxue.org/jianjie_'

def mainFun(n):
    listUrl = url + str(n + 1) + '.html'
    content = url_content(listUrl)
    if content is False:
        return False
    try:
        title = content.find(class_='son1').text
    except:
        return False
    author = content.find(class_='son2').find('p').text
    contents = content.find(class_='son2').text.replace(author, '').replace('作者：', '')
    novel = postNovelMethod({'title': title, 'cate': '1', 'pic': 'pic', 'author': author, 'content': contents, 'url': listUrl})
    bookconts = content.find_all(class_='bookcont')
    sort = 1
    for bookcont in bookconts:
        contentUrls = bookcont.find_all("a")
        for contentUrl in contentUrls:
            contUrl = baseUrl + contentUrl.attrs['href']
            detailContent = url_content(contUrl)
            name = detailContent.find(class_='son1').text
            detail = str(detailContent.find(class_='novel-read'))
            print(detail)
            postChapterMethod({'novel': novel, 'sort': sort, 'name': name, 'content': detail, 'url': contUrl})
            sort = sort + 1
